  ...
  return (
    <div className="flex min-h-[80vh] items-center">
      <div className="shell">
        ...
        <h1 className="mt-6 font-display text-[clamp(2.6rem,6vw,5rem)] text-ink">{t.errors.notFoundTitle}</h1>
        <p className="mt-5 max-w-md text-sm text-ink/60">{t.errors.notFoundText}</p>
        <Link
          href={`/${DEFAULT_LOCALE}`}
          className="mt-10 inline-block bg-champagne-deep px-8 py-4 text-[0.7rem] uppercase tracking-[0.2em] text-surface transition-colors hover:bg-ink"
        >
          {t.errors.backHome}
        </Link>
      </div>
